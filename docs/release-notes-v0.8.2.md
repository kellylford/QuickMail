...
The unread count shown next to each folder was a snapshot taken when QuickMail connected, and it did not change afterward — reading messages, new mail arriving, or a sync would not update it until you reconnected. Now the counts refresh shortly after you mark messages read, after new mail arrives, and after a sync completes. On Gmail, where reading one message can change several folders' counts at once (Inbox, All Mail, and the message's labels), all of the affected counts update together. The counts update in place, so your position in the folder list is not disturbed, and the count is spoken as part of each folder's name.
...
### Folder unread-count refresh (issue #227, PR #229)

- Folder `UnreadCount` was set only at folder-list load and never updated after mark-read / new mail / sync. A debounced, server-authoritative refresh now re-queries IMAP `STATUS` after those events (coalescing bursts) and writes counts onto the existing folder models, updating the tree nodes **in place** — no tree rebuild, so folder-tree keyboard focus is preserved. Server-authoritative (not optimistic decrement) so Gmail's cross-label `\Seen` propagation is reflected across every affected folder.
- Independent review found no Critical/High. Its one significant catch was accessibility: the count is carried in the folder's `AutomationProperties.Name`, not only `ItemStatus` — a screen-reader user confirmed that a count in `ItemStatus` alone is not announced. The visible label stays count-free (the count shows as a badge), and `AutomationName` is a computed value refreshed in place, so the count is both announced and kept current without doubling.
