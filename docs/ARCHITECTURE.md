...
**ConnectivityService** (#637) is the app's one answer to "are we online?". Two inputs: the machine-level signal from `NetworkChange.NetworkAvailabilityChanged` (behind `INetworkAvailabilitySource`, so tests need no network card) and the outcome of real operations per account, fed by `MainViewModel.ApplyAccountStatus` (the chokepoint every connect outcome passes through) and the folder, message and attachment fetch paths via `NoteOperationOutcome`. The verdict is network-up AND not-every-known-account-offline: one reachable account keeps the app online while the account list shows the others' state. Going offline is debounced five seconds so a blip never announces itself; coming back publishes at once. The service is told, never subscribes to the backends itself. `MainViewModel` derives `ConnectionStatusText` from its verdict plus a phase (`ConnectionStatusFor`), announces "Offline. Showing cached messages." / "Back online." once each, reconnects every offline account when the network returns, retries on a backoff (30 s doubling to 5 min) while the network is up but nothing answers, and drops an account from `_connectedAccountIds` when it goes unreachable — the one place an id leaves that set other than account removal. The Outbox drains on the same events. Null in ui-probe mode, which is structurally offline and must not say so in its captures.
...
Groups (`GroupModel`) are flat, local-only, and keyed by an incrementing integer `Id` with a list of `MemberContactIds`. `LoadAllGroupsAsync` returns groups sorted by `LastUsedTicks` descending; `TouchGroupAsync` bumps the timestamp so a freshly-inserted group sorts to the top. The model exposes computed `ResolvedMemberCount` and `MissingContactCount` (both `[JsonIgnore]`) and a human-readable `Display` ("Name, N members" / "(M missing)" / "Empty group") so the UI does not have to recompute them. The full group surface is on `IContactService`: `LoadAllGroupsAsync`, `CreateGroupAsync`, `RenameGroupAsync`, `DeleteGroupAsync`, `AddMemberAsync`, `RemoveMemberAsync`, `ListGroupsForContactAsync`, `TouchGroupAsync`. A corrupt `groups.json` is renamed to `groups.json.bak-{timestamp}` and treated as empty, the same recovery `FlagService` and `RowLayoutService` use. Client-side rules deliberately do not: see RuleService below.

**RuleService** loads/saves client-side mail rules from `rules.json`. Each `MailRule` has AND-combined conditions (FromContains, ToContains, SubjectContains, BodyContains, MustHaveAttachments), the four text ones each gated by a `Use*Condition` flag, and one or more actions (MarkAsRead, MarkAsUnread, CopyToFolder, MoveToFolder, Delete). `Action` is the main one; `Actions` lists them all when there is more than one (#682), and `AllActions()` returns them in the order they run — marking, then copying, then moving or deleting. `Action` is the last of them that a build from before #682 understands — the move or delete, else the marking, and `CopyToFolder` only for a copy-only rule — so such a build, which reads that field alone, does the most of the rule it can rather than nothing. `ApplyRulesAsync` runs on incoming Inbox mail; `ApplyRulesToExistingAsync` runs on demand over the Inbox of the accounts it is handed, and the rules window hands it only the selected account. Every rule belongs to one account (`AccountId`); a legacy rule with a null `AccountId`, an old "all accounts" rule, is copied to each non-Graph account on the first load once accounts exist, and dropped with a log line when there is none. It is never copied onto a Graph mailbox. File writes use an atomic temp-then-rename pattern. A `rules.json` that is there but can't be read or parsed is neither renamed nor read as empty: `LoadRules` throws `RulesFileUnreadableException` (#700), because every writer loads the whole list, changes it and saves it back, so an empty read made the next save wipe the file. Only a missing file or folder counts as no rules. `SaveRules` reads first when nothing has been read yet, so it throws rather than replace such a file. The failure is not cached, so a briefly locked file recovers on the next read. Sync treats it as no rules; the Rules Manager's status line and the status bar report it; a Microsoft 365 conversion remaps saved views and settings and leaves the rules to a later launch. Server-side rules on work/school Microsoft 365 accounts go through **GraphServerRuleService** (the Graph `messageRule` API) instead. `UnifiedRulesViewModel` lists both kinds for one account at a time, and `ServerRuleEditorViewModel` is the one editor for both. A new rule is classified as server or client when it is first saved; an edit keeps the rule's kind.
...
**Offline is not a mode.** `--online` means the local store is never initialised; being offline means the store is the only source that answers. They are orthogonal: `--online` with no network shows nothing, says "Offline." rather than "Offline. Showing cached messages.", and has no Outbox. Every new local-store call still follows the two-scope fetch pattern below, whichever state the app is in.
