...
**AutoDiscoverService** (`IAutoDiscoverService`) finds IMAP/SMTP settings for an address in four tiers, returning the first hit and `null` when all are exhausted: (1) the local `ProviderCatalog` — offline, instant, nothing leaves the machine; (2) Mozilla's autoconfig database at `autoconfig.thunderbird.net`, which receives only the domain; (3) the domain's own Exchange **Autodiscover** endpoint, tried at `autodiscover.<domain>` then `<domain>`, which receives the full address as Outlook does; (4) Microsoft's `getuserrealm.srf` lookup, queried with a fixed `discover@` local part so only the domain leaves the machine. Tier 4 exists because most Microsoft 365 tenants use a custom domain and publish no Autodiscover record QuickMail can reach — `icanbrew.com` is the worked example: ISPDB 404s and both Autodiscover hosts fail to resolve, so tiers 1–3 all miss and the user was left being asked for an IMAP host that does not exist. It is treated as a **suggestion**, not a configuration (`DiscoverySource.MicrosoftRealm`): the lookup answers "does this domain have an Entra ID tenant", which a company running its own mail on Microsoft 365 internally also answers yes to, so the ViewModel words it as "appears to be". A `NameSpaceType` of Managed or Federated counts only when `DomainName` matches the queried domain — gmail.com and yahoo.com answer Federated with `DomainName` `live.com`, and must not be mistaken for tenants. Tiers 2–4 are skipped entirely when `AutoDiscoverOnline` is off. HTTPS only, 4 s per request and 15 s overall, and every failure mode — DNS, TLS, 404, timeout, malformed XML or JSON — falls through to the next tier rather than propagating. Automatic redirects are off (a hop could downgrade to HTTP) but 30x responses are followed manually, HTTPS-only, up to 3 hops, because Microsoft 365 custom domains commonly answer Autodiscover with a 302. Discovered settings that would authenticate in the clear are rejected outright — `socketType` `plain`, Autodiscover `<SSL>off</SSL>`, and SMTP port 25 — because `UseSsl=false` maps to MailKit's `StartTlsWhenAvailable`, which connects in plaintext when the server advertises no STARTTLS. It owns an `HttpClient` and is disposed in `App.OnExit`. The Autodiscover client here is also the prerequisite for shared-mailbox detection (#31).
