...
**What a client-side rule can use.** A client-side rule can test **From addresses**, **Subject contains**, **Sender contains**, **Sent to addresses**, **Subject or body contains**, **Body contains**, and **Has attachments**. **From addresses** and **Sent to addresses** each take several addresses, and **Sender contains** can be used together with **From addresses**, in which case both have to match. It has a single subject condition, so **Subject contains** and **Subject or body contains** cannot both be used in one rule; **Save** says so if you try. It can **Mark as read** or **Mark as unread**, **Copy to folder**, and **Move to folder** or **Delete**, and it can do more than one of them — but not mark a message both read and unread, or both move and delete it. It does them in that order: marking first, then copying, then moving or deleting, since after that the message is no longer in the Inbox. **Mark as unread** changes only this computer's copy of the message, so it cannot be combined with moving or deleting, which would throw that copy away and the mark with it. A client-side rule cannot copy into the Inbox either: it runs on the Inbox, so each copy would be copied again on the next check. Everything else in the editor needs a server-side rule: **Sent to me**, **Sent only to me**, **Importance is**, **Set importance to**, **Forward to**, and **Stop processing more rules**. **Mark as unread** is the reverse — only a client-side rule can do it, which is why its label says so. Editing never changes a rule's kind, so a client-side rule stays client-side. The editor offers the server-side options even where they cannot be used — on an account with client-side rules only, and when you edit a client-side rule — and **Save** refuses a rule that uses one. Either way it names what is in the way: the server-side option to remove, or the two conditions that cannot be used together. While you edit a server-side rule, **Mark as unread** is unavailable, because the server has no way to do it.
