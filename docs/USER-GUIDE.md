...
- **Save Draft** answers "Draft saved on this computer. It will go to the server when you are back online." rather than reporting a failure. If a save or a send is refused — no sender account chosen, attachments over 25 MB, an account whose Drafts folder has never synced, or a save that could reach neither this computer nor the server; and, for Send only, no recipient, an address QuickMail cannot use, or no stored password — the reason appears at the top of the compose window and the cursor is put on it. A save refused that way leaves the window open, so this is what tells you why it will not close.
- Auto-save shows "Auto-saved on this computer" with the time, instead of "Auto-saved". If auto-save ever cannot write to your computer at all, the message exists nowhere — so the compose window says so at the top, where a refused draft's reason appears, and the next successful save clears it.
...
- Closing the compose window is never blocked by being offline. Answering **No** to the save prompt removes the copy auto-save had already written for a message you started in that window — declining to keep a message means it is not kept, and not quietly uploaded later. For a draft you merely *opened*, **No** leaves that draft where it is: any changes auto-save had already written to your computer are kept, declining the prompt does not undo them, and they will upload with the draft. If you were online and auto-save had already put a copy on the server, that copy stays there too — **No** removes only what is on this computer.
- Moving, copying and archiving are refused while it is waiting to upload: there is nothing on the server to act on, and QuickMail shows a message saying so rather than half-doing it. The same happens if you try to convert an account to Microsoft 365 while it still holds drafts that have not gone up — that conversion clears the local mailbox, so it stops rather than destroy them. The refusal happens when you give the command, before any folder picker opens, so you are not asked to choose a destination for something that will not move. It covers the whole selection — if you had ordinary messages selected too, none of them move either, so take the draft out of the selection first. (A draft your server has refused is turned down for a different reason: it will not upload at all until you edit and save it.)
...
QuickMail uploads these drafts on its own — when the account connects, and again on every background sync while QuickMail is running, so you do not need to do anything. (If you have set mail syncing to "Off (server push only)" in Settings, the sweep is what that turns off: queued drafts then go up the next time the account connects, which for most people means the next time QuickMail starts.) When it does, the status bar says how many went up — briefly, since the folder counts from the same sync replace it a moment later. The lasting record is the row itself: once a draft is on the server it stops saying **Not on server**. And if you were editing a draft that already existed on the server, the copy up there is replaced rather than duplicated. A draft you have open in a compose window is left alone while it is open, so an upload does not land on top of what you are still typing.

If your server refuses one — a message it will not accept, an address it objects to — that draft stops being retried and its row says **not uploaded** instead of "not on server". It does not block the drafts behind it, which upload normally. **Open it and QuickMail shows you what the server said**, at the top of the compose window, and puts you straight on it — so you can fix what it objected to and save again, which puts the draft back in the queue. If the reason begins "QuickMail could not upload it" rather than naming your server, the fault is QuickMail's own; the draft is still safe on your computer, and saving it again is what tries once more.

Some failures are not about any one draft: a sign-in your server will not accept, a secure connection it will not complete, or an account with no Drafts folder on the server. Nothing would upload for that account whatever you did to the drafts, so QuickMail stops, leaves every draft queued and still reading "not on server", and tells you what needs putting right — usually signing in again in **Manage Accounts** — and puts that on the rows themselves, so the drafts read **not uploaded** with the reason until it is sorted out. Nothing is lost and nothing is taken out of the queue: they start going up again on their own on the next sync once the account is working. A draft with nothing wrong opens where it always did. A draft can also fail because its saved copy on your computer has gone. Its row reads **not uploaded** just like a refused one, but opening it answers "That draft could not be opened: its saved copy on this computer is missing." The message cannot be recovered — the row is all that is left, and deleting it is all there is to do. If instead it answers "its saved copy on this computer could not be read", try once more: usually the store was simply busy. If it fails again the saved copy is damaged, and that draft cannot be recovered either. A draft that will not open for any other reason answers "That draft could not be opened. Nothing has been discarded" — most often that is simply being offline with a draft that lives on the server, and it opens normally once you are back.

One thing to know before you edit: while the compose window is open, a refusal from **Send** replaces the reason your server gave for the draft. The server's reason is not lost — it is on the row, and reopening the draft brings it back — but it is no longer on screen.
...
Alongside what you type, an in-app report (options 1 and 2) adds a short **Environment** section so a problem can be reproduced in the right context: the QuickMail version, your Windows version, the .NET runtime version, the active color theme, the current view, the current sort order, the message open mode (reading pane, tab, or window), and how many accounts you have set up together with the mail protocols those accounts connect over — for example `2 (IMAP, Microsoft 365)`. Any shared mailboxes are counted separately, as in `2 (Microsoft 365), plus 1 shared mailbox`, since a shared mailbox is not one of your own accounts, and the protocol list covers your own accounts only. That last line is counts and protocol names only: no address, server name, or account name is included.
...
| Field | Speaks |
|-------|--------|
| Not on server | "not on server" for a draft waiting to upload, "not uploaded" for one that will not go up — refused by your server, stopped by a problem in QuickMail itself, or with its saved copy missing |
