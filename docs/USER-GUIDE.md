...
**What a client-side rule can use.** A client-side rule can test **From addresses** (one address), **Subject contains**, **Sender contains** (but not together with From addresses), **Sent to addresses** (one address), **Body contains**, and **Has attachments**. It can **Mark as read** or **Mark as unread**, **Copy to folder**, and **Move to folder** or **Delete**, and it can do more than one of them — but not mark a message both read and unread, or both move and delete it. It does them in that order: marking first, then copying, then moving or deleting, since after that the message is no longer in the Inbox. **Mark as unread** changes only this computer's copy of the message, so it cannot be combined with moving or deleting, which would throw that copy away and the mark with it. A client-side rule cannot copy into the Inbox either: it runs on the Inbox, so each copy would be copied again on the next check. Everything else in the editor needs a server-side rule: the other conditions and actions, and more than one address in a field. **Mark as unread** is the reverse — only a client-side rule can do it, which is why its label says so. Editing never changes a rule's kind, so a client-side rule stays client-side. The editor offers the server-side options even where they cannot be used — on an account with client-side rules only, and when you edit a client-side rule — and **Save** refuses a rule that uses one. For a new rule it names the option to remove; for an edited one it asks you to remove what a client-side rule cannot do. While you edit a server-side rule, **Mark as unread** is unavailable, because the server has no way to do it.
...
- The status bar says how it is going: "Messages: 1,240 of 2,000 downloaded", and "Messages: downloading 120 of 500" while a batch is coming down. Press **Ctrl+9** to reach the status bar and **Right** to move along it. Nothing is said aloud as it changes; it is there to be read when you want it.
