...
Some failures are not about any one draft: a sign-in your server will not accept, a secure connection it will not complete, or an account with no Drafts folder on the server. Nothing would upload for that account whatever you did to the drafts, so QuickMail stops, leaves every draft queued and still reading "not on server", and says once in the status bar what needs putting right — usually signing in again in **Manage Accounts**. Your drafts start going up again on their own once it is. A draft with nothing wrong opens where it always did. A draft can also fail because its saved copy on your computer has gone. Its row reads **not uploaded** just like a refused one, but opening it answers "That draft could not be opened: its saved copy on this computer is missing." The message cannot be recovered — the row is all that is left, and deleting it is all there is to do. If instead it answers "its saved copy on this computer could not be read", try once more: usually the store was simply busy. If it fails again the saved copy is damaged, and that draft cannot be recovered either. A draft that is already on your server answers differently again — "there is no copy on this computer and the server could not be reached" — and that one is only about being offline: it opens normally once you are back.
