...
Archiving works from every view. In the **From**, **To**, and **Conversations** groupings, archiving a group moves the whole group at once, the same way Delete does. Archive is also on the message context menu (Shift+F10) and in the command palette, and the **Ctrl+Shift+M** shortcut can be changed in **Settings → Keyboard**.
...
| `Ctrl+F` | Forward |
| `Delete` | Delete |
| `Ctrl+Shift+M` | Archive (move to the account's Archive folder) |
| `Ctrl+Q` | Mark as Read |
