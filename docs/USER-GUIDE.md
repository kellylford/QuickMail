...
- **Save Draft** answers "Draft saved on this computer. It will go to the server when you are back online." rather than reporting a failure. If a save or a send is refused — no sender account chosen, or attachments over 25 MB — the reason appears at the top of the compose window and the cursor is put on it, so the window never just refuses to close without saying why.
