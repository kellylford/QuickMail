...
- **Save Draft** answers "Draft saved on this computer. It will go to the server when you are back online." rather than reporting a failure. If a save or a send is refused — no recipient, no sender account chosen, an address QuickMail cannot use, no stored password, attachments over 25 MB, or an account whose Drafts folder has never synced — the reason appears at the top of the compose window and the cursor is put on it. A save refused that way leaves the window open, so this is what tells you why it will not close.
...
If your server refuses one — a renamed Drafts folder, a message it will not accept — that draft stops being retried and its row says **not uploaded** instead of "not on server". It does not block the drafts behind it, which upload normally. **Open it and QuickMail shows you what the server said**, at the top of the compose window, and puts you straight on it — so you can fix what it objected to and save again, which puts the draft back in the queue. A draft with nothing wrong opens where it always did. A draft can also fail because its saved copy on your computer has gone. Its row reads **not uploaded** just like a refused one, but opening it answers "That draft could not be opened: its saved copy on this computer is missing." The message cannot be recovered — the row is all that is left, and deleting it is all there is to do. If instead it answers "this computer's mail store could not be read just now", nothing has been lost: the store was busy. Wait a moment and open it again.
