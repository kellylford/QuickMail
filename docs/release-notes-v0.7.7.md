...
- When you are up to date, the entry reads **"No updates available."** Activating it opens the QuickMail releases page in your browser, so the entry is always there and useful — you don't have to infer from the absence of a notification that the check happened.
- When an update is found, the entry changes to **"Update available: version X.Y.Z"** — activating it opens that release's page directly — and a spoken announcement fires a few seconds after launch: "QuickMail update available: version X.Y.Z. Check the Help menu." This respects your action-result announcement preference.
...
The Help-menu entry is always present: `UpdateAvailableText` defaults to "No updates available" and the entry binds to `OpenUpdatePageCommand`, which opens the specific release when one is known (`UpdateReleaseUrl` non-empty) or the general releases page otherwise. The startup check (`CheckForUpdateInBackgroundAsync`) updates the entry's text and stays silent when up to date (announcing only a found update). The status-bar update button's visibility tracks `UpdateReleaseUrl` so it appears only when an update actually exists.
