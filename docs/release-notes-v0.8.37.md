...
**Alt+A now also works while composing.** It moves focus to the attachment list of the message you are writing, matching what it does in an open message. **Ctrl+Shift+A** is still the way to add files, and the new command is in the compose Command Palette as **Focus Attachment List**, so it can be given a different shortcut in Settings → Keyboard.
...
- `RadioGroupNavigationTests` (10) feed real `KeyEventArgs` through the attached handler rather than synthesizing keystrokes, so they are deterministic and need no `QUICKMAIL_RUN_INPUT_TESTS`.
