...
- **A new rule belongs to the account its editor opened on.** The Rules Manager's **Account** list
  ...
  offered the second account's folders. The editor now uses the account it opened on throughout.
...
- **Editing a server-side rule no longer drops Mark as unread without a word.** A server-side rule
  can't mark mail unread, so ticking it and saving closed the editor as if it had worked and left the
  rule unchanged. **Save** now says to remove it.
