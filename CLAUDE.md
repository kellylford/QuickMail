...
- **Folder picker**: `FolderPickerWindow` has two presentations. `useTreeView: true` opens a folder tree — use it wherever the user is choosing a destination folder they already know from the folder tree (move/copy message, move/copy folder; issues #250, #431), and default new destination pickers to it. The flat virtualized list with a search box (`/` moves focus to search) is what "Go to Folder" needs, being the only picker that also offers the virtual folders (All Inboxes, All Mail, …). The rule editors (`RulesManagerWindow`, `ServerRuleEditorWindow`) still open the flat list and are candidates for the tree. **A picker must not open with nothing selected** — a tree with no selection announces the tree and no item, and leaves the user to work out that Down is what starts things. Every destination picker passes `initialFolder`, the folder the user came from; tree mode stands something in when that folder is not in the tree (the parent it is being moved out of, else the first real folder), and never opens on a header or path segment, which carry no folder and leave `Open` disabled. The two rule editors pass no `initialFolder` and so open on the first row — fix that if you move them to the tree.
- **Folder destinations are account-scoped**: the mail backends move and copy by *name* over the source account's connection and never look at the destination's `AccountId`. Offering another account's folders is therefore not merely useless — with a name collision ("Archive") it silently acts on the wrong folder. `FolderPickerWindow.ForFolderMoveCopy` scopes to the one owning account; any new destination picker must too. (`MainWindow.BuildMessageFolderPicker` scopes to the owning account**s** — plural, since a selection can span accounts — so the collision is still reachable there when messages from two accounts are moved at once. Not fixed here; do not copy that shape for a single-source picker.)
