...
public class ClientRulesOnGraphTests : IDisposable
{
    ...
    private sealed class RecordingMailService : IMailService
    {
        ...
        public Task<IReadOnlyList<(string Id, DateTimeOffset ReceivedUtc)>> GetFolderMessageIdDatesAsync(Guid a, string f, CancellationToken ct = default) => Task.FromResult<IReadOnlyList<(string, DateTimeOffset)>>([]);
